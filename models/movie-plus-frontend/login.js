...
function setLampState(isOn) {
	...
	elements.body.classList.add("is-transitioning");
	window.setTimeout(() => {
		elements.body.classList.remove("is-transitioning");
	}, 280);
...
function updateLampHint() {
	if (!state.isLampOn) {
		elements.authHint.textContent = "Enciende la lampara para revelar el acceso premium.";
		return;
	}

	elements.authHint.textContent = state.activeTab === "login"
		? "La luz esta encendida. Ingresa y vuelve al catalogo."
		: "La luz esta encendida. Crea tu cuenta para continuar.";
}
