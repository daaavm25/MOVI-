...
function getTmdbApiKey() {
    if (!TMDB_API_KEY) {
        const error = new Error('TMDB_API_KEY no configurada en el entorno.');
...
app.get('/peliculas/:id', async (req, res) => {
    try {
        ...
        const response = await axios.get(`${TMDB_BASE_URL}/movie/${movieId}`, {
            params: { api_key: getTmdbApiKey(), language: 'es-MX' }
        });
        const m = response.data;
        return res.json({
            id: m.id,
            titulo: m.title,
            categoria: (m.genres || []).map(g => g.id),
            imagen: m.poster_path ? `https://image.tmdb.org/t/p/w500${m.poster_path}` : null,
            fecha: m.release_date || null,
...
app.post('/watchlist', async (req, res) => {
    try {
        ...
        const { external_id, titulo } = req.body;
        const id_usuario = sessionUser.id;
        if (!external_id || !titulo) {
            return res.status(400).json({
        ...
        const nueva = await Watchlist.create({ ...req.body, id_usuario });
...
app.get('/watchlist', async (req, res) => {
    try {
        ...
        const where = { id_usuario: sessionUser.id };
...
app.delete('/watchlist/:id', async (req, res) => {
    try {
        ...
        const item = await Watchlist.findOne({ where: { id } });
        if (!item) {
            return res.status(404).json({
                error: 'Elemento no encontrado'
            });
        }
        if (item.id_usuario !== sessionUser.id) {
